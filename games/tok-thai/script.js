...
function startGame() {
  ...
  feedback.textContent = "発音を見切って選びましょう。正解ごとに速度が0.1ずつ上がります。";
  renderStones();
  updateHud();
  spawnWord();
}
...
function answer(choice) {
  ...
  if (choice === state.current.roman) {
    ...
    feedback.textContent = `正解: ${state.current.thai} = ${state.current.roman}`;
...
function missCurrent(reason, choice = "") {
  ...
  feedback.textContent = reason === "time"
    ? `時間切れ: ${missed.thai} は ${missed.roman}`
    : `惜しい: ${missed.thai} は ${missed.roman}`;
  fallingWord.classList.add("hidden");
  state.current = null;
  renderStones();
  updateHud();
...
function finishGame() {
  ...
  wellMessage.innerHTML = "<strong>Clear!</strong><span>15問クリアしました</span>";
  wellMessage.classList.remove("hidden");
  resultPanel.classList.remove("hidden");
  resultText.textContent = state.mistakes.length
    ? `石化した単語は ${state.mistakes.length} 件です。下で正しい発音を確認できます。`
    : "ミスなしです。短いタイ文字への反応、かなりいい感じです。";
  renderMistakes();
  updateHud();
}
...
function generateFallbackDistractors(roman) {
  ...
  vowels.forEach((vowel) => {
    if (roman.includes(vowel)) {
      variants.add(roman.replace(vowel, `${vowel}${vowel}`));
      variants.add(roman.replace(`${vowel}${vowel}`, vowel));
    }
  });
  ["h", "ng", "n", "t", "k"].forEach((ending) => variants.add(`${roman}${ending}`));
  if (roman.length > 2) variants.add(roman.slice(0, -1));
